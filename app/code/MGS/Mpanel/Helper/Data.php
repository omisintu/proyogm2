...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
	...
	public function getThemeSettings(){
		return [
			...
			'catalogsearch'=> 
			[
				'per_row' => $this->getStoreConfig('instantsearch/additional_product/product_per_row')
			],
			'catalog_brand'=> 
			[
	...
	public function getImageMinSize($ratio = NULL) {
        ...
        switch ($ratio) {
            // 1/1 Square
            case 1:
                $result = array('width' => 150, 'height' => 150);
                break;
            // 1/2 Portrait
            case 2:
                $result = array('width' => 150, 'height' => 300);
                break;
            // 2/3 Portrait
            case 3:
                $result = array('width' => 150, 'height' => 225);
                break;
            // 3/4 Portrait
            case 4:
                $result = array('width' => 150, 'height' => 200);
                break;
            // 2/1 Landscape
            case 5:
                $result = array('width' => 150, 'height' => 75);
                break;
            // 3/2 Landscape
            case 6:
                $result = array('width' => 150, 'height' => 100);
                break;
            // 4/3 Landscape
            case 7:
                $result = array('width' => 150, 'height' => 113);
                break;
        }
    ...
    public function getMainColorSetting($storeId, $themeName) {
		...
		if (is_readable($themeStyleFile)){
			$parsedArray = $this->_parser->load($themeStyleFile)->xmlToArray();
			if(isset($parsedArray['class_setting']['main'])){
				foreach($parsedArray['class_setting']['main'] as $configNote=>$classAttribute){
					foreach($classAttribute as $attribute=>$classString){
						$attribute = str_replace('_', '-', $attribute);
						$arrAttribute[$classString] = [
							$attribute => $this->getStoreConfig('color/main/'.$configNote, $storeId)
						];
						$setting = array_merge_recursive($setting,$arrAttribute);
						$arrAttribute = [];
					}
	...
	public function convertPerRowtoCol($perRow = NULL){
		if(!$perRow){
			$settings = $this->getThemeSettings();
			$perRow = $settings['catalog']['per_row'];
		}
	...
	public function convertColClass($col, $type){
		...
		if($type=='col'){
			if(($col=='custom-5' || $col=='custom-7' || $col=='custom-8')){
				return 'col-lg-'.$col. ' col-md-3 col-sm-4 col-xs-6';
			}else{
				if($col == 12 || $col == 6 || $col == 4){
					$class = 'col-lg-'.$col.' col-md-'.$col;
				}else{
					$class = 'col-lg-'.$col.' col-md-3';
				}
				if($col==12){
	...
	public function getFooterClass(){
        ...
        if($footer == 'footer' || $footer == '') { $header = "footer_1"; }
		$class = str_replace('_', '', $footer);
		if($this->_acceptToUsePanel){
    ...
    public function getSearchTreeCategory($categoryId, $parentName = NULL){
        ...
        if($categoryId == $this->getCurrentlySelectedCategoryId()){
            $class .= ' active';
        }
		if($parentName != null){
            $html[] = '<span class="'.$class.'" data-id="'.$category->getId().'" data-name="'.$parentName.'/'.$category->getName().'">'.$category->getName().'</span>';
        }else{
            $html[] = '<span class="'.$class.'" data-id="'.$category->getId().'" data-name="'.$category->getName().'">'.$category->getName().'</span>';
        }
		...
		if($childrenCount>0){
			foreach ($children as $child) {
                ...
                if($_child->getIsActive()){
                    $htmlChildren .= $this->getSearchTreeCategory($child->getId(), $category->getName());
                }
			}
		}
		if (!empty($htmlChildren)) {
    ...
    public function convertContent($layoutContent, $builderContent = NULL){
		$beginMain = '<div class="page-wrapper">';
			...
			if (count($themeColorSetting) > 0) {
				foreach ($themeColorSetting as $class => $style) {
                    ...
                    if (count($style) > 0) {
                        $html .= $class . '{';
                        foreach ($style as $_style => $value) {
                            if($_style == 'box-shadow-0'){
                                $html .= 'box-shadow: inset 0 0 0 0 '. $value .';';
                            }elseif($_style == 'box-shadow-1') {
                                $html .= 'box-shadow: inset 0 50px 0 0 '. $value .';';
                            }elseif($_style == 'box-shadow-2') {
                                $html .= 'box-shadow: inset 0 100px 0 0 '. $value .';';
                            }else {
                                $html .= $_style . ': ' . $value . ';';
                            }
                        }
